...
def update_admin(admin_id: int, admin_data: AdminCreate, db: Session):
    db_admin = db.query(AdminModel).filter(AdminModel.admin_id == admin_id).first()
    if not db_admin:
        raise HTTPException(status_code=404, detail="Admin not found")
...
def search_admins(db: Session, hospital_id: Optional[int] = None, search: Optional[str] = None):
    query = db.query(AdminModel)
    if hospital_id:
        query = query.filter(AdminModel.hospital_id == hospital_id)
    if search:
